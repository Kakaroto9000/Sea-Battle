...
namespace SeaBattle
{
    public class Arena
    {
        ...
        private bool CheckIsCanPlaceHere(int NumbOfShip)
        { 
            if (Ships[NumbOfShip].rotation == false)
            {
                for (int i = 0; i < Ships[NumbOfShip].lenghts; i++)
                {
                    if (PreviosCoardinate[GetShipCord("x", NumbOfShip), GetShipCord("y", NumbOfShip) + i] == '#')
                    {
                        return false;
                    }
                    else if (PreviosCoardinate[GetShipCord("x", NumbOfShip) - 1, GetShipCord("y", NumbOfShip) + i] == '#')
                    {
                        return false;
                    }
                    else if (PreviosCoardinate[GetShipCord("x", NumbOfShip) + 1, GetShipCord("y", NumbOfShip) + i] == '#')
                    ...
                    for(int j = 0; j < 3; j++)
                    {
                        if (PreviosCoardinate[GetShipCord("x", NumbOfShip) - 1 + j, GetShipCord("y", NumbOfShip) - 1] == '#')
                        {
                            return false;
                        }
                        else if (PreviosCoardinate[GetShipCord("x", NumbOfShip) - 1 + j, GetShipCord("y", NumbOfShip) + Ships[NumbOfShip].lenghts] == '#')
                        {
                            return false;
                        }
                        else return true;
            ...
            else
            {
                for (int i = 0; i < Ships[NumbOfShip].lenghts; i++)
                {
                    if (PreviosCoardinate[GetShipCord("x", NumbOfShip) + i, GetShipCord("y", NumbOfShip)] == '#')
                    {
                        return false;
                    }
                    else if (PreviosCoardinate[GetShipCord("x", NumbOfShip) + i, GetShipCord("y", NumbOfShip) + 1] == '#')
                    {
                        return false;
                    }
                    else if (PreviosCoardinate[GetShipCord("x", NumbOfShip) + i, GetShipCord("y", NumbOfShip) - 1] == '#')
                    ...
                    for (int j = 0; j < 3; j++)
                    {
                        if (PreviosCoardinate[GetShipCord("x", NumbOfShip) - 1, GetShipCord("y", NumbOfShip) - 1 + j] == '#')
                        {
                            return false;
                        }
                        else if (PreviosCoardinate[GetShipCord("x", NumbOfShip) + Ships[NumbOfShip].lenghts, GetShipCord("y", NumbOfShip) - 1 + j] == '#')
                        {
                            return false;
                        }
                    }
